...
function SwipeCards({ photos, setPhotos }) {
    ...
    <View style={StyleSheet.absoluteFill}>
      {photos
        .map((item, index) => (
          <Animated.View
            key={index}
            style={[index === 0 ? animatedCardStyle : {}, StyleSheet.absoluteFill]}
            {...(index === 0 ? panResponder.panHandlers : {})}
          >
            <Image key={index} source={{ uri: "file://" + item.path }} style={[StyleSheet.absoluteFill]} />
          </Animated.View>
        ))
        .reverse()}
    </View>
